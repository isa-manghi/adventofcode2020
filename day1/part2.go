...
func main() {
	num1, num2, num3 := numberlooppart2()
	fmt.Println("num1: %x, num2: %x, num3: %x", num1, num2, num3)
	mult := num1 * num2 * num3
...
func numberlooppart2() (int, int, int) {
	...
	for _, line := range lines {
		...
		for _, innerline := range lines {
			...
			for _, innerinnerline := range lines {
				innerinnernum, _ := strconv.Atoi(innerinnerline)
				if innernum+intnum+innerinnernum == 2020 {
					return innerinnernum, innernum, intnum
				}
			}
		}
	}
